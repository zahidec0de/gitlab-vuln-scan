...
Find out what version of GitLab a server is running, and check it against
known CVEs — no login required, no nmap required.
...
=== gitlab.example.com:443 ===
  ...
  Version    : 17.4.2  (confirmed floor)
...
What's still there, on almost every version: the hash of GitLab's own
front-end asset bundle, at `/assets/webpack/manifest.json`. The browser has
to fetch this file before login even works, so it's always public. This
tool reads that hash and looks up which GitLab version(s) produced it.

Sometimes more than one version shares the exact same hash — usually when
a patch release only changed backend code, not the front end. When that
happens, the tool tells you the lowest version it could be (called the
"floor") and offers a second command (`verify_version.py`) to pin down the
exact one.
...
- Python 3.9 or newer (no extra packages needed for `scan.py` and
  `verify_version.py`)
- Internet access — the tools talk to the target server, and sometimes to
  gitlab.com and Docker Hub to look things up
- The `gitlab_hashes.json` and `gitlab_cves.json` files that come with this
  repo (already included, kept up to date automatically — see below)

Only the `automation/` update scripts need an extra package: `pip install requests`.
...
**"A vendor/report says a server is running version X — I want to confirm or disprove that."**
```
...
## `scan.py` — detect a version, optionally check CVEs
...
2. The build commit shown at `/users/sign_in` (only present on some
   versions — GitLab has been phasing it out).
3. If step 2 found something, it's checked directly against gitlab.com's
   own records — exact, and always up to date.
...
| `NEEDS VERIFICATION` | scan.py wasn't sure of the exact patch, and the possible versions include both safe and vulnerable ones |

Every result also shows the raw data it was based on (hashes, URLs, API
responses) so you can double-check it yourself with `curl`, without
trusting this tool blindly.
...
| `--no-insecure` | verify TLS certificates (off by default — many internal servers use self-signed certs) |
...
Hub (no need to install Docker) and compares it byte-for-byte against what
the server returns. If they match, it's confirmed — not guessed.

```
$ python3 verify_version.py --target gitlab.example.com:443 --version 17.4.2 --edition ee
CONFIRMED: gitlab.example.com:443 is running GitLab ee 17.4.2
...
Exit code: `0` confirmed, `1` mismatch, `2` error (e.g. server unreachable).

---

## `gitlab_version.nse` — Nmap version
...
Use `scan.py` instead if you want the CVE check, the gitlab.com lookup, or
JSON output — this script doesn't have those.
...
automatically once a day (via GitHub Actions, see `.github/workflows/main.yml`):

- **`gitlab_hashes.json`** — asset hash → GitLab version. Rebuilt by
...
- **`gitlab_cves.json`** — GitLab CVEs and the exact versions they affect.
  ...
  straight from NVD (the U.S. National Vulnerability Database) rather
  than depending on a third-party lookup at scan time.
...
with CVE reporting inspired by
